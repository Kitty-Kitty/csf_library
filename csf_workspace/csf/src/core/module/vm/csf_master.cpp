...
csf::core::base::csf_int32 csf_master::init() {
	...
	//初始化虚拟机对象
	tmp_bool_ret = tmp_vm.init(csf_nullptr);
...
csf::core::base::csf_int32 csf_master::start() {
	...
	//在共享内存中创建虚拟机对象
	set_vm(get_sm().create_object<csf_vm>(get_name(), 1));
 ...
 *    非0 ：表示失败；
 */
csf::core::base::csf_int32 csf_master::process_option() {
 ...
 *    false :  表示失败；
 */
csf_bool csf_master::init_shared_memory() {
	...
	//生成共享内存的名称
	csf_snprintf(tmp_buf, csf_sizeof(tmp_buf), "master_%s", get_name().c_str());

	tmp_int_ret = get_sm().create(tmp_buf, csf_sizeof(csf_vm));
