		...
		namespace system
		{
			...
			public:
				...
				*/
				...
				/**
				* 该函数主要根据引导文件内容创建app的所属所有设备模块。
				* 返回：true表示初始化配置信息成功；false表示初始化配置文件失败。
				*
				* @param app    表示目标app对象    表示所属的app信息
				* @param configure_manager    表示解析配置文件信息后，需要保存的目标对象configure_manager
				*/
				csf_bool init_devices(csf::core::module::csf_app& app
					, csf::core::system::csf_configure_manager& configure_manager);
				/**
				* 该函数主要用于初始化app。
				* 返回：true表示初始化配置信息成功；false表示初始化配置文件失败。
				...
				*/
				csf_bool init_device(csf::core::module::csf_app& app
					, csf_element& element);
				/**
				* 该函数主要用于初始化app。
				* 返回：true表示初始化配置信息成功；false表示初始化配置文件失败。
				...
				* @param configure_manager    表示解析配置文件信息后，需要保存的目标对象configure_manager
				*/
				csf_bool init_device_ioes(csf::core::module::csf_device& device, csf_element& element
					, csf::core::system::csf_configure_manager& configure_manager);
				/**
				* 该函数主要用于初始化app。
				* 返回：true表示初始化配置信息成功；false表示初始化配置文件失败。
				...
				* @param element    表示当前的device节点内容
				*/
				csf_bool init_device_io(csf::core::module::csf_device& device, csf_element& element);
				/**
				 * 表示当前的app_configure文件作用的app对象
				 * 
