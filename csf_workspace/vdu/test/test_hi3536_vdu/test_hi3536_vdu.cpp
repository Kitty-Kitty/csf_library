...
// 		tmp_format.setStencilBufferSize(8);
	...
	try
	{
		...
		if (tmp_view) {
			...
			//tmp_qgl_widget->setFixedSize(QSize(16, 16));

#if 0
			tmp_pb = new QGLPixelBuffer(QSize(3840, 2160), QGLFormat(QGL::Rgba | QGL::AlphaChannel | QGL::DepthBuffer | QGL::DoubleBuffer | QGL::AccumBuffer | QGL::DirectRendering));
			tmp_pb->makeCurrent();
#endif

#if 0
			//tmp_qgl_widget->setFixedSize(QSize(1920, 1080));
			...
			tmp_view->update();
 			...
 			tmp_fb = new QGLFramebufferObject(3840, 2160, fmt);
			if (!tmp_fb) {
				printf("create QGLFramebufferObject() failed!\r\n");
				return -1;
			}
			else {
				tmp_fb->bind();
